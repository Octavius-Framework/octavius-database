...
Nullability is determined by the type parameter you pass — no separate `assertNotNull()` call needed:
...
- Non-null `T` + result is null → `Failure(ConversionException(UNEXPECTED_NULL_VALUE))`
