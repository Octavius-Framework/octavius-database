...
private fun generateDeveloperMessage(
    ...
    return when (messageEnum) {
        StepDependencyExceptionMessage.DEPENDENCY_ON_FUTURE_STEP -> "Step attempts to use a result from a future or current step ${
            args.getOrNull(
                0
            )
        }."
        ...
        StepDependencyExceptionMessage.TRANSFORMATION_FAILED -> "User-defined transformation (.map {}) failed for value derived from step $stepIndex. Cause: ${
            args.getOrNull(
                0
            )
        }"
