...
internal class PostgresSqlPreprocessorTest {
    ...
    inner class BasicParsing {
        ...
        fun `should find a single named parameter`() {
            ...
            Assertions.assertThat(result).containsExactly(ParsedParameter("userId", 31, 38))
        }
    ...
    inner class IgnoredConstructs {
        ...
        fun `should ignore parameters inside single-quoted strings`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("userId", 43, 50))
        }
        ...
        fun `should ignore parameters inside single-quoted strings, even with escaped quotes`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("realId", 72, 79))
        }
        ...
        fun `should ignore parameter inside E-string literal and find parameter outside`() {
            ...
            Assertions.assertThat(result).containsExactly(ParsedParameter("real", 67, 72))
        }
        ...
        fun `should ignore parameters inside double-quoted identifiers`() {
            ...
            Assertions.assertThat(result)
                .containsExactly(ParsedParameter("realParam", expectedStart, expectedStart + 10))
        }
        ...
        fun `should ignore parameters inside single-line comments`() {
            ...
            """.trimIndent()
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("userId", 56, 63))
        }
        ...
        fun `should ignore parameters inside multi-line comments`() {
            ...
            """.trimIndent()
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("category", 121, 130))
        }
        ...
        fun `should ignore postgres type cast operator`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("id", 55, 58))
        }
        ...
        fun `should ignore postgres operators starting with at symbol`() {
            ...
            Assertions.assertThat(result).containsExactly(ParsedParameter("id", expectedStart, expectedStart + 3))
        }
    }
    ...
    inner class DollarQuotedStrings {
        ...
        fun `should ignore parameters inside simple dollar-quoted strings`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("realId", 53, 60))
        }
        ...
        fun `should ignore parameters inside tagged dollar-quoted strings`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("real", 75, 80))
        }
    }
    ...
    inner class ArrayAndSliceHandling {
        ...
        fun `should ignore colon in array slice when using at-prefix`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("id", 38, 41))
        }
        ...
        fun `should find parameter used as array index`() {
            ...
            val result = PostgresSqlPreprocessor.parse(sql)
            Assertions.assertThat(result).containsExactly(ParsedParameter("index", 11, 17))
        }
    ...
    inner class QuestionMarkEscaping {
        ...
        fun `should escape a single question mark`() {
            ...
            Assertions.assertThat(result).isEqualTo("SELECT * FROM t WHERE json_col ?? 'key'")
        }
        ...
        fun `should escape multiple question marks`() {
            ...
            val result = PostgresSqlPreprocessor.escapeQuestionMarks(sql)
            Assertions.assertThat(result).isEqualTo("SELECT * FROM t WHERE json_col ??| array['a', 'b'] AND other ??& array['c']")
        }
        ...
        fun `should NOT escape question marks inside single-quoted strings`() {
            ...
            val result = PostgresSqlPreprocessor.escapeQuestionMarks(sql)
            Assertions.assertThat(result).isEqualTo("SELECT 'is this a question?' as q FROM t WHERE col ?? 'key'")
        }
        ...
        fun `should NOT escape question marks inside E-string literals`() {
            ...
            val result = PostgresSqlPreprocessor.escapeQuestionMarks(sql)
            Assertions.assertThat(result).isEqualTo("SELECT E'escaped \\' quote and question?' FROM t WHERE col ?? 'key'")
        }
        ...
        fun `should NOT escape question marks inside dollar-quoted strings`() {
            ...
            val result = PostgresSqlPreprocessor.escapeQuestionMarks(sql)
            Assertions.assertThat(result).isEqualTo("SELECT $$ why? $$ FROM t WHERE col ?? 'key'")
        }
